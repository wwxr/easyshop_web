...
* @company Finedo.cn
* @author Finedo
* @date 2018年7月25日 上午9:42:58
* @version V1.0  
*/
;function FinedoContextMenu(options) {
    this.opt = this.extend(true, this.options, options);
    this.opt.target = document.getElementById(this.opt.target);
    ...
    if (this.opt.classes && !this.opt.linkClass) {
        ...
        this.menuTitleClass = this.extend(true, {},
        this.opt.classes.menuTitle.self);
        this.menuTitleIconClass = this.extend(true, {
            display: "inline-block"
        },
        this.opt.classes.menuTitle.icon);
        this.menuTitleConClass = this.extend(true, {},
        this.opt.classes.menuTitle.content);
        this.itemClass = this.extend(true, {},
        this.opt.classes.item.self);
        this.itemIconClass = this.extend(true, {
            display: "inline-block"
        },
        this.opt.classes.item.icon);
        this.itemConClass = this.extend(true, {},
        this.opt.classes.item.content);
        this.hoverClass = this.extend(true, {},
        this.opt.classes.item.hover);
        this.separatorClass = this.extend(true, {},
        this.opt.classes.separator);
...
FinedoContextMenu.prototype.init = function() {
    this.menuBox = document.createElement("div");
    this.attr(this.menuBox, {
        "unselectable": "on"
    });
    this.attr(this.menuBox, {
    ...
    if (items) {
        for (i = 0; i < items.length; i++) {
            ...
            if (items[i] === "-") {
                ...
                var self = this; (function(i) {
                    if (!self.opt.linkClass) {
                        self.on(item, "mouseover",
                        function() {
                            self.css(this, self.hoverClass);
                        });
                        self.on(item, "mouseout",
                        function() {
                            self.css(this, self.itemClass);
                        });
                    }
                    self.on(item, "mousedown",
                    function(e) {
                        self.sb(e);
...
FinedoContextMenu.prototype.show = function() {
...
FinedoContextMenu.prototype.hide = function() {
...
FinedoContextMenu.prototype.bind = function() {
    var self = this;
    self.on(self.opt.target, "contextmenu",
    function(e) {
        var oEvent = e || event;
        nocontextmenu(oEvent);
        return false;
    });
    self.on(self.opt.target, "mousedown",
    function(e) {
        if (e.button == 2) {
            self.show(e);
        }
    ...
    self.on(document, "mousedown",
    function(e) {
        if (e.button != 2) {
            self.hide();
        }
    });
    self.on(self.menuBox, "click",
    function(e) {
        self.sb(e);
    });
    if (self.opt.autoHide) {
        var timer = null;
        self.on(self.menuBox, "mouseover",
        function(e) {
            clearTimeout(timer);
        });
        self.on(self.menuBox, "mouseout",
        function(e) {
...
FinedoContextMenu.prototype.$ = function(q, o) {
    var self = this;
    ...
    case '.':
        ...
        var c = s.substr(1);
        for (var i = 0; i < l.length; i++) if (l[i].className.search('\\b' + c + '\\b') != -1) obj.push(l[i]);
        break;
    default:
    ...
    if (val) {
        var l = [];
        var a = val.split('=');
        for (var i = 0; i < obj.length; i++) if (a.length == 2 && obj[i][a[0]] == a[1]) l.push(obj[i]);
        obj = l;
    }
    if (attr) {
        var l = [];
        for (var i = 0; i < obj.length; i++) if (obj[i][attr]) l.push(obj[i]);
        obj = l;
    }
    if (ss.length > 1) {
        var l = [];
        for (var i = 0; i < obj.length; i++) {
            ...
            else for (var j = 0; j < ll.length; j++) l.push(ll[j]);
        }
        obj = l;
    }
    if (sObj && ss.length == 1) {
...
FinedoContextMenu.prototype.on = function(element, eventName, handler) {
...
FinedoContextMenu.prototype.options = {
    target: document,
    hasIcon: false,//是否需要图标
    hasTitle: false,//是否需要标题
    autoHide:true,//是否自动隐藏右键菜单
    linkClass: false,//是否使用外链样式，如果为true，则配置样式失效;若选择false且同时设置css样式，则配置样式与css样式按优先级呈现;若选false,则可不用配置classes属性
    menu: {
    },
    classes: {//对有菜单的样式控制，当linkClass设置为true时，可传这种值来覆盖默认样式
        menuBox: {//右键菜单的样式
            "width": "200px", "background": "#FEFEFE", "border": "solid 1px #333", "cursor": "pointer", "border-radius": "10px", "overflow": "hidden", "box-shadow": "-1px -1px 5px #000,1px 1px 5px #ccc", "-webkit-box-shadow": "-1px -1px 5px #000,1px 1px 5px #ccc", "-moz-box-shadow": "-1px -1px 5px #000,1px 1px 5px #ccc", "-o-box-shadow": "-1px -1px 5px #000,1px 1px 5px #ccc", "font-size": "14px"
        },
        menuTitle: {//菜单标题的样式
            self: {//标题整体样式
                "background": "#DDDDDD", "padding": "10px 5px",
                "border-bottom": "inset #000 1px", "cursor": "default"
            },
            icon: {//标题图标样式
            ...
            content: {//标题内容样式
                height: "22px", "line-height": "22px", "padding-left": "10px", "color": "#333333", "font-size": "16px", "display": "inline-block", "vertical-align": "top"
            }
        },
        item: {//菜单项样式控制
            self: {//菜单项整体样式控制
                "background": "#FFF", "padding": "3px 3px"
            },
            icon: {//菜单项图标样式
                width: "22px", height: "22px"
            },
            content: {//菜单项内容样式
                height: "22px", "line-height": "22px", "padding-left": "10px", "color": "#333333", "font-size": "14px", "display": "inline-block", "vertical-align": "top"
            },
            hover: {//菜单项鼠标移入样式
                background: "#ccc"
            },
        ...
        separator: {//分隔符样式
            width: "100%", height: "1px", border: "solid 1px #000", "border-left": "none", "border-right": "none"
        }
    }
};
FinedoContextMenu.prototype.extend = function(flag, destination, source) {
    if (flag) {
        var obj = {};
...
FinedoContextMenu.prototype.css = function() {
    var arg = arguments;
    var i = 0;
    var len = arg.length;
    if (len == 2) {
...
FinedoContextMenu.prototype.attr = function() {
    var arg = arguments;
    var i = 0;
    var len = arg.length;
    if (len == 2) {
...
FinedoContextMenu.prototype.isArray = function(obj) {
    var result;
    try {
...
FinedoContextMenu.prototype.sb = function(e) {
...
FinedoContextMenu.prototype.hasClass = function(obj, cls) {
    return obj.className.match(new RegExp('(\\s|^)' + cls + '(\\s|$)'));
};
FinedoContextMenu.prototype.addClass = function(obj, cls) {
    if (this.isArray(obj)) {
        for (var i = 0; i < obj.length; i++) {
...
FinedoContextMenu.prototype.removeClass = function(obj, cls) {
    if (this.isArray(obj)) {
        for (var i = 0; i < obj.length; i++) {
...
FinedoContextMenu.prototype.toggleClass = function(obj, cls) {
    if (this.isArray(obj)) {
        for (var i = 0; i < obj.length; i++) {
            if (this.hasClass(obj[i], cls)) {
...
FinedoContextMenu.prototype.isEmptyObject = function(obj) {
    for (var name in obj) {
        return false;
    }
    return true;
