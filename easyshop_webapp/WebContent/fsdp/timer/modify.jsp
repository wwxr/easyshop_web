...
<div>
	<div class="add-common-head">
        ...
        <input type="button" class="finedo-button-blue" style="float:right" value="提交" onclick="timermodify.doNext();">
    </div>
    
    <form method="post" id="ajaxForm" name="ajaxForm">
    <input type="hidden" id="id" name="id" value="">
    <input type="hidden" id="optrid" name="optrid" value="">
    	...
    	<div class="finedo-nav-title">基本信息</div>
	   	<ul class="finedo-ul">
			...
			<li>
				...
				<input class="finedo-text" type="text" onfocus="finedo.showhintinfo('name');" id="name" name="name" value="" style="width:600px;">
                <span for="name" class="finedo-hint-info"></span>
                <span for="name" class="finedo-hint-error"></span>
                <span for="name" class="finedo-hint-right"></span>
			</li>

           	<li>
           		<span class="finedo-label-title"><font color=red>*</font>Bean类名</span>
           		<input class="finedo-text" type="text" onfocus="finedo.showhintinfo('beanname');" id="beanname" name="beanname" value="">
                <span for="beanname" class="finedo-hint-info">请填写Bean类名，如：CleanerTimer,注意不需要前面的package名</span>
                <span for="beanname" class="finedo-hint-error"></span>
                <span for="beanname" class="finedo-hint-right"></span>
           	</li>

           	<li>
           		<span class="finedo-label-title"><font color=red>*</font>Bean类方法名</span>
           		<input class="finedo-text" type="text" onfocus="finedo.showhintinfo('method');" id="method" name="method" value="">
                <span for="method" class="finedo-hint-info">请填写需要执行的Bean类方法名，如：doWork</span>
                <span for="method" class="finedo-hint-error"></span>
                <span for="method" class="finedo-hint-right"></span>
           	</li>
           	
           	<li>
           		<span class="finedo-label-title"><font color=red>*</font>CRON</span>
           		<input class="finedo-text" type="text" onfocus="finedo.showhintinfo('cron');" id="cron" name="cron" value="">
                <span for="cron" class="finedo-hint-info">如: 每5分钟执行一次示例: * 0/5 * * * *, 格式说明: 秒 分 时 日 月 星期, 其中以空格间隔</span>
                <span for="cron" class="finedo-hint-error"></span>
                <span for="cron" class="finedo-hint-right"></span>
           	</li>
		</ul>
		 <ul>
	    	<li class="add-center"><input type="button" class="finedo-button" value="提    交" onClick="timermodify.dosubmit()" ></li>
	    </ul>
...
var timermodify={
    inittimer:function(){
        var timerid = finedo.fn.getParameter("id");
        $("#id").val(timerid);
        finedo.action.ajax({
            ...
            "callback":function(retdata){
                ...
                $("#name").val(timertask.name);
                $("#beanname").val(timertask.beanname);
                $("#method").val(timertask.method);
                $("#cron").val(timertask.cron);
            }
        });
    },
    checkdata:function() {
     ...
     doNext:function() {
         timermodify.dosubmit();
     },
     dosubmit:function() {
         if(!timermodify.checkdata()) 
             return;

         finedo.action.ajaxForm({
             url:"../../finedo/timer/modify",
             form:"ajaxForm",
             callback:timermodify.submitcallback
         });
     },
