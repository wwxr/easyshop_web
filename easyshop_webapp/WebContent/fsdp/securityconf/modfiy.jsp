...
<div>
    <div class="add-common-head">
        ...
        <input type="button" class="finedo-button-blue" style="float:right" value="提交" onclick="securitymodify.dosubmit();">
    </div>
    ...
    <div id="common_add_div" >
        ...
        <ul>
            <li class="add-center"><input type="button" class="finedo-button" value="提    交" onClick="securitymodify.dosubmit()" ></li>
        </ul>
    </div>
    </form>
</div>
...
var securitymodify={
    initsecurity:function(){
        var passwdtype = finedo.fn.getParameter("passwdtype");
        finedo.action.ajax({
            "url":"../../finedo/securityconf/querybyid",
            "data":{"passwdtype":passwdtype},
            "callback":function(retdata){
                if(retdata.fail){
                    finedo.message.error(retdata.resultdesc);
                    return;
                }
                var securityconf = retdata.object;
                $("#passwdtype").val(securityconf.passwdtype);
                $("#validday").val(securityconf.validday);
                $("#minlength").val(securityconf.minlength);
                $("#maxlength").val(securityconf.maxlength);
                $("#specialnum").val(securityconf.specialnum);
                $("#capitalnum").val(securityconf.capitalnum);
                $("#lowercasenum").val(securityconf.lowercasenum);
                $("#digitalnum").val(securityconf.digitalnum);
                $("#remark").val(securityconf.remark);
            }
        });
    },
    //数据验证
