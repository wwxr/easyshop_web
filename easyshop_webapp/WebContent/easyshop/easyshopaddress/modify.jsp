...
<div>
	<div class="ylnav">
	<span>修改地址信息</span><input type="button" value="" class="back" onClick="(function(){window.parent.$('.finedo-dialog-mask').remove();window.parent.$('.finedo-dialog').remove();})()">
    </div>
    <input hidden='hidden' id='addressid'>
    ...
    <div class="ylsubmit"><input type="button" class="ylbtn" value="提    交" onClick="easyshopaddressmodify.dosubmit()" ></div>
</div>

<script language="javascript">
var easyshopaddressmodify={};
/**
 * 初始化加载要修改的数据
 */
easyshopaddressmodify.initmodify=function(){
	//模拟城市-联动/搜索
	...
	var keyvalue=finedo.fn.getParameter("addressid");
	$("#addressid").val(keyvalue);
	finedo.action.ajax({
        "url":"../../finedo/easyshopaddress/querybyid",
        "data":{"addressid":keyvalue},
        "callback":easyshopaddressmodify.queryCallback
    });
 ...
 * 数据查询回调方法
 */
easyshopaddressmodify.queryCallback=function(ajaxret){
	if(ajaxret.fail){
        finedo.message.error(retdata.resultdesc);
        return;
    }
    var retobject = ajaxret.object;
    $("#addressid").val(retobject.addressid);
    $("#userid").val(retobject.userid);
    $("#address").val(retobject.address);
    $("#isdefault").val(retobject.isdefault);
    $("#username").val(retobject.username);
    $("#phone").val(retobject.phone);
   	$("input[name='userProvinceId']").prev().text(retobject.province)
	city:$("input[name='userCityId']").prev().text(retobject.city)
	area:$("input[name='userDistrictId']").prev().text(retobject.area)
	provincenum:$("input[name='userProvinceId']").val(retobject.provincenum)
	citynum:$("input[name='userCityId']").val(retobject.citynum)
	areanum:$("input[name='userDistrictId']").val(retobject.areanum)
};
/**
 * 数据修改提交
 */
easyshopaddressmodify.doNext=function() {
	easyshopaddressmodify.doSubmit();
};
/**
 * 数据修改提交
 */
 easyshopaddressmodify.checkdata=function(){
	 var result=true;
 ...
 * 提交方法
 */
 easyshopaddressmodify.dosubmit=function() {
	if(!easyshopaddressmodify.checkdata()) 
		return;
	finedo.action.ajax({
		url:"../../finedo/easyshopaddress/modify",
		data:{
...
LazyLoad.js([
    ...
    easyshopaddressmodify.initmodify();
});
</script>
</body>
</html>