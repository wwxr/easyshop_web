...
<div>
    ...
    <div class="ylsubmit"><input type="button" class="ylbtn" value="提    交" onClick="easyshopgoodsmodify.dosubmit()" ></div>
</div>

<script language="javascript">
var easyshopgoodsmodify={};
var up=[];
var upvideo=[];
//图片路径
var uppath=[];
 ...
 * 初始化加载要修改的数据
 */
easyshopgoodsmodify.initmodify=function(){
	var keyvalue=finedo.fn.getParameter("goodsid");
	$("#goodsid").val(keyvalue);
	finedo.action.ajax({
        "url":"../../finedo/easyshopgoods/querybyid",
        "data":{"goodsid":keyvalue},
        "callback":easyshopgoodsmodify.queryCallback
    });
};
/**
 * 数据查询回调方法
 */
easyshopgoodsmodify.queryCallback=function(ajaxret){
	if(ajaxret.fail){
        finedo.message.error(retdata.resultdesc);
        return;
    }
    easyshopgoodsmodify.initgoodstype();  
    ...
    $("#presentprice").val(retobject.presentprice);
    $("#originalprice").val(retobject.originalprice);
    $("#goodsname").val(retobject.goodsname);
    $("#isrecommend").val(retobject.isrecommend);
    $("#goodstypeid").val(retobject.goodstypeid);
    ...
    if(finedo.fn.isNotNon(retobject.imgids)){
    	var imgs=retobject.imgids.split(",");
...
easyshopgoodsmodify.checkdata=function(){
	var result=true;
 ...
 * 数据修改提交
 */
easyshopgoodsmodify.dosubmit=function() {
	if(!easyshopgoodsmodify.checkdata()) 
		return;
	var upstr='';
	...
	var paramdetail = paramdetail.getContent();
	finedo.action.ajax({
		url:"../../finedo/easyshopgoods/modify",
		data:{
			goodsid:$("#goodsid").val().trim(),//商品id
...
function uploadcomplete1(fileobj){
	...
	console.log(uppath.toString())
	console.log(upthumpath.toString())
...
//初始化商品类型
easyshopgoodsmodify.initgoodstype=function() {
	finedo.action.ajax({
...
LazyLoad.js([
    ...
    easyshopgoodsmodify.initmodify();
});
</script>
</body>
</html>